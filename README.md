...
- **预览独立卡片（v1.9.1）**：点开文件后预览在独立浮动卡片中打开（面板内底部预览区退役，树区占满高度）；卡片可拖动、四边四角缩放、双击标题栏最大化/还原、`Esc` 或 × 关闭；同一时间单个卡片复用；全部预览能力（渲染/编辑、行号、继续加载）在卡片内可用
- **树内搜索过滤（v1.9.0）**：`Ctrl+F` / `⌘F` 呼出搜索条，即时过滤已加载节点（保留目录层级）；命中子串高亮、`↑↓` 键盘导航、`Enter` 打开、匹配计数与空状态；跟随「显示隐藏」开关
- **Markdown 渲染预览（v1.9.0）**：`.md` 文件默认富文本渲染（GFM 表格 / 任务列表 / 删除线、代码块语法高亮 + 一键复制、可折叠标题目录、限宽居中阅读）；`渲染 / 编辑` 切换（`Ctrl+[` / `Ctrl+]`），按文件类型记忆上次模式
- **IDE 式实时高亮编辑（v1.9.9 起）**：打开代码/文本文件直接进入高亮可编辑视图（无需切编辑模式）——透明输入层叠在语法高亮层上，输入即所见高亮；`pre-wrap` 自动换行（行号锚定逻辑行首）；行号与预览一致；`Tab` 缩进、`Ctrl+S` 保存、`Esc` 退出；代码文件默认高亮 + 行号（服务端 marked + highlight.js 渲染）；单行超长（>2000 字符）截断并标记；≤2000 行高亮、>4MB 分段「继续加载」；深浅主题自动切换高亮配色
...
- **v1.9.10**：删除独立的「源码 / 纯文本」只读视图模式（编辑器整合后冗余）；markdown 保留 [渲染, 编辑]，代码/文本为单一 [编辑]；模式按钮仅在多模式时显示；截断文件回退只读纯文本行视图。
- **v1.9.9**：text-overlay 语法高亮编辑器——打开代码/文本文件直接进入高亮可编辑视图（无需切编辑模式）；透明 textarea 覆盖在高亮层上，输入即所见高亮；`pre-wrap` 自动换行；行号与预览一致（同一 table 行布局）；滚动 transform 同步。
- **v1.9.8**：预览卡片缩放改为 4 边条 + 4 角块共 8 个 grip（此前四角太小难抓），hover 高亮。
- **v1.9.7**：恢复预览卡片与帮助浮层的毛玻璃效果（重叠根因是行高塌陷，已由 table 布局修复，毛玻璃回归安全）。
- **v1.9.6**：行视图改用原生 `<table>` 布局（tr 高度 = max(td)，内容软换行撑高整行、行号锚定逻辑行首——从机制层面消除内容与行号错位/重叠；虚拟化用 colspan spacer 行；视图根元素带 `data-fexp-version="1.9.6"` 便于确认加载版本）。
