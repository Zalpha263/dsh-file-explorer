...
- **ui-beautify 插件面板适配**：检测到 ui-beautify 时自动注册为「插件面板」的一员（名称「文件浏览器」），两种模式统一由插件面板管理——卡片模式占停靠卡、经典模式右侧停靠 / 浮动；标题栏入口统一为「插件面板」按钮，不再显示独立的「📁 文件」按钮（ui-beautify 未安装时回退为自带按钮 + 面板）
- **悬浮球**：侧边可拖动入口，可随时开关（卡片模式下自动隐藏，由卡片宿主浮动能力替代）
