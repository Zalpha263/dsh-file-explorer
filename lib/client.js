...
`;
		...
		async function apply(ctx) {
			...
			/* --- v1.10.2: dock 绑定改用规范写法 ---
			   规范：dsh-ui-beautify/docs/plugin-panel-integration.md（dock API v2）。
			   ① 首选 cordis 可选依赖 `ctx.inject(['dock'], cb)`：dock 出现时执行回调
			      （注册面板 + 订阅引擎变化），dock 消失（ui-beautify 热重载/卸载）时
			      执行回调返回的清理函数，dock 再次出现时自动重新执行 —— 不再需要
			      身份比对，也不再需要靠 800ms 跟随循环去「碰运气」重绑；
			   ② 守护 ctx（运行时动态包）没有 `inject`，退化为 internal/service 事件 +
			      1s 兜底轮询的**幂等**绑定器（用 dock.has(id) 判断是否需要重绑）。 */
			/** Bind the panel to one dock host (idempotent). Returns true on success. */
			function attachDock(d) {
				if (dockApiRef === d && dockDispose !== null) return true;
				if (dockApiRef !== undefined) detachDock();
				dockApiRef = d;
				try {
			...
			/** Release the host binding and hand the open state back to the standalone
			 *  panel (mirrors onDockChange's "leaving host mode" branch). */
			function detachDock() {
				const d = dockApiRef;
				if (d === undefined) return;
				if (typeof d.isOpen === "function") { try { state.open = !!d.isOpen(PANEL_ID); } catch (err) {} }
			...
			function bindDockFallback() {
				...
				const sync = function () {
					...
					if (dockApiRef !== undefined) {
						const alive = typeof d.has === "function" ? d.has(PANEL_ID) : d === dockApiRef;
						if (alive) return;
						detachDock();
					}
					attachDock(d);
				...
				sync();
				try { off = ctx.on("internal/service", function (name) { if (name === "dock") sync(); }); } catch (err) { off = null; }
			...
			ctx.effect(function followLoop() {
				const id = window.setInterval(() => {
					/* v1.10.2: dock 绑定已交给 ctx.inject / 退化绑定器（它们各自负责
					   「服务出现时绑定」），这里只保留 host 模式的周期性再同步。 */
			...
			/* v1.10.2: dock 绑定 —— 规范写法优先（cordis 可选依赖），守护 ctx 退化为
			   事件 + 轮询的幂等绑定器。规范见
