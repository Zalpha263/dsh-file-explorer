		...
		async function apply(ctx) {
			...
			function deletePath(path, type, name) {
				...
				remote().fsDelete(path).then((res) => {
					...
					for (const key of [...state.expanded]) if (key.toLowerCase().startsWith(prefix)) state.expanded.delete(key);
					for (const key of [...state.cache.keys()]) if (key.toLowerCase().startsWith(prefix)) state.cache.delete(key);
					renderTree();
