...
/* --- 帮助浮层 --- */
.fexp-help { position: fixed; z-index: 2147483300; width: 360px; max-width: calc(100vw - 24px); border: 1px solid var(--dsw-alias-border-l2); border-radius: 12px; background: var(--dsw-alias-bg-layer-2); box-shadow: var(--dsw-shadow-lv3, 0 10px 40px rgba(0,0,0,.28)); color: var(--dsw-alias-label-primary); padding: 14px 16px; font-size: 13px; animation: fexp-pop 180ms cubic-bezier(0.34, 1.56, 0.64, 1); }
...
/* ============================================================
   ...
   ============================================================ */
.fexp-preview-card { position: fixed; z-index: 2147483400; display: flex; flex-direction: column; min-width: 480px; min-height: 320px; background: var(--dsw-alias-bg-layer-2); border: 1px solid var(--dsw-alias-border-l2); border-radius: 12px; box-shadow: var(--dsw-shadow-lv3, 0 12px 48px rgba(0,0,0,.32)); overflow: hidden; animation: fexp-card-in 180ms cubic-bezier(0.34, 1.56, 0.64, 1); }
...
.fexp-card-grip:hover { background: color-mix(in srgb, var(--dsw-alias-brand-primary) 22%, transparent); border-radius: 4px; }
/* ============================================================
   v1.9.3：行视图 —— grid 行容器布局
   每逻辑行一个 grid 行（行号列 + 内容列 1fr）：行高由内容列决定，
   软换行时行号锚定逻辑行首、不占新行号、不与文字重叠。
   ============================================================ */
.fexp-lines { display: flex; flex-direction: column; flex: 1; min-width: 0; }
.fexp-linerow { display: grid; align-items: start; }
.fexp-linerow .fexp-lineno { grid-column: 1; padding-right: 10px; text-align: right; color: var(--dsw-alias-label-tertiary); user-select: none; font-size: 12px; line-height: 20px; box-sizing: border-box; overflow: hidden; background: inherit; }
.fexp-lines .fexp-cline { grid-column: 2; padding-left: 8px; min-width: 0; }
`;
		...
		async function apply(ctx) {
			...
			function buildLineView(p, htmlLines) {
				...
				rows.className = "fexp-lines fexp-code-font";
				...
				/**
				 * 渲染 [first, first+count) 逻辑行。每行一个 flex 行容器
				 * （行号 span + 内容 span）：软换行时内容 span 撑高整行，
				 * 行号 span 随行等高 —— 行号与逻辑行一一对应、不占新行号。
				 * 虚拟化时用上下 spacer 撑出真实滚动高度。
				 */
				const renderRange = (first, count) => {
					...
					if (first > 0) {
						...
						sp.style.height = (first * LINE_H) + "px";
						sp.style.flex = "none";
						frag.appendChild(sp);
					}
					for (let i = 0; i < count; i += 1) {
						const n = first + i;
						const row = document.createElement("div");
						row.className = "fexp-linerow";
						row.style.minHeight = LINE_H + "px";
						// grid 两列：行号列 + 内容列（1fr）；行高由内容列决定，
						// 软换行时行号始终锚定逻辑行首 —— 不会与文字重叠
						row.style.gridTemplateColumns = noW + "px 1fr";
					...
					const rest = total - (first + count);
					if (rest > 0) {
						const sp = document.createElement("div");
						sp.style.height = (rest * LINE_H) + "px";
						sp.style.flex = "none";
						frag.appendChild(sp);
					}
				...
				if (total <= VIRTUALIZE_THRESHOLD) {
					...
					// 虚拟化：只渲染可视窗口 ± 缓冲行；上下 spacer 撑出滚动高度
					const onScroll = () => {
			...
			function ensurePreviewCard() {
				...
				previewCardEl = card;
				// 四角缩放 grip（pointer capture 会把 move/up 重定向到 grip 自身）
				for (const mode of ["se", "ne", "sw", "nw"]) {
			...
			function onCardResizeMove(e) {
				...
				if (mode === "se") { width = clamp(width + dx, CARD_MIN_W, window.innerWidth - 16); height = clamp(height + dy, CARD_MIN_H, window.innerHeight - 16); }
				else if (mode === "ne") { width = clamp(width + dx, CARD_MIN_W, left + width - 8); height = clamp(height - dy, CARD_MIN_H, window.innerHeight - 16); top = rect.top + (rect.height - height); }
				else if (mode === "sw") { width = clamp(width - dx, CARD_MIN_W, rect.left + width - 8); left = rect.left + (rect.width - width); height = clamp(height + dy, CARD_MIN_H, window.innerHeight - 16); }
				else if (mode === "nw") { width = clamp(width - dx, CARD_MIN_W, rect.left + width - 8); left = rect.left + (rect.width - width); height = clamp(height - dy, CARD_MIN_H, window.innerHeight - 16); top = rect.top + (rect.height - height); }
				previewCardEl.style.left = left + "px";
				previewCardEl.style.top = top + "px";
