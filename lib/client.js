...
`;
		...
		async function apply(ctx) {
			...
			function renderPreview() {
				...
				if (!p) {
					if (previewCardEl) {
						// 只隐藏并移除内容区；保留缩放 grip（textContent 清空会连 grips 一起删掉，
						// 导致再次打开时无法缩放）
			...
			// ============ v1.9.9 text-overlay 高亮编辑器 ============
			// 结构：行号 table（行号 td + 高亮内容 td，pre-wrap 自动换行，行高随
			// 内容）+ 透明 textarea 覆盖其上（文字透明、光标可见、pre-wrap 换行）。
			// 高亮层与 textarea 使用相同的字体/行高/宽度/换行规则 → 输入即所见
			// 高亮，光标行与高亮行逐行对齐；textarea 滚动时高亮层 transform 同步。
			function buildEditor() {
				...
				const htmlLines = (p.data && Array.isArray(p.data.lines)) ? p.data.lines : null; // ≤2000 行服务端高亮
				const digits = String(lineCount).length;
				const noW = 14 + digits * 8; // 行号列宽
				const wrap = document.createElement("div");
				...
				body.style.cssText = "flex:1;min-height:0;display:flex;flex-direction:column;position:relative;overflow:hidden;";
				// 高亮层：行号 + 高亮内容的 table（自动换行撑高，行号锚定逻辑行首）
				const table = document.createElement("table");
				table.className = "fexp-linetable fexp-code-font";
				table.style.cssText = "width:100%;margin-top:8px;table-layout:fixed;border-collapse:collapse;";
				const tbody = document.createElement("tbody");
				table.appendChild(tbody);
				const frag = document.createDocumentFragment();
				for (let i = 0; i < lineCount; i += 1) {
					...
					no.className = "fexp-lineno";
					no.style.width = noW + "px";
					no.style.paddingTop = "0";
					no.textContent = String(i + 1);
					const tx = document.createElement("td");
					tx.className = "fexp-cline";
					tx.style.paddingTop = "0";
					const line = segments[i] !== undefined ? segments[i] : "";
					if (htmlLines) tx.innerHTML = htmlLines[i] !== undefined ? htmlLines[i] : "";
				...
				// 交互层：透明 textarea（pre-wrap 自动换行，光标可见）
				const ta = document.createElement("textarea");
				ta.className = "fexp-code-font";
				ta.style.cssText = "position:absolute;inset:0;width:100%;height:100%;margin:0;padding:8px 12px 8px " + (noW + 18) + "px;border:none;outline:none;resize:none;background:transparent;color:transparent;caret-color:var(--dsw-alias-label-primary);white-space:pre-wrap;word-break:break-word;overflow-wrap:anywhere;overflow:auto;box-sizing:border-box;";
				ta.value = ed.text;
				ta.spellcheck = false;
				ta.wrap = "soft";
				ta.addEventListener("input", () => { ed.text = ta.value; });
				// 滚动同步：textarea 滚动 → 高亮层整体上移（text-overlay 标准做法）
				ta.addEventListener("scroll", () => {
					table.style.transform = "translateY(-" + ta.scrollTop + "px)";
				}, { passive: true });
