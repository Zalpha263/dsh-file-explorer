		...
		async function apply(ctx) {
			...
			function walkTree(container, path, depth) {
				...
				for (const e of dirs) {
					...
					row.addEventListener("dragenter", (ev) => onDragEnter(ev, e.path));
			...
			function deletePath(path, type, name) {
				...
				remote().fsDelete(path).then((res) => {
					...
					for (const key of [...state.expanded]) if (key.toLowerCase() === prefix || key.toLowerCase().startsWith(prefix + sep)) state.expanded.delete(key);
					for (const key of [...state.cache.keys()]) if (key.toLowerCase() === prefix || key.toLowerCase().startsWith(prefix + sep)) state.cache.delete(key);
					renderTree();
			...
			function onDragEnter(ev, dirPath) {
				if (dragSrcPath === null) return;
				ev.preventDefault();
				if (isSelfOrDescendant(dragSrcPath, dirPath)) return;
				clearDropTarget();
				dropTargetEl = ev.currentTarget;
				dropTargetEl.classList.add("fexp-drop-target");
			}
			function onDragLeave(ev) {
				if (dropTargetEl === ev.currentTarget) clearDropTarget();
			}
			function onDrop(ev, dirPath) {
				...
				clearDropTarget();
				dragSrcPath = null;
				if (isSelfOrDescendant(src, dirPath)) {
					showToast("不能移动到自身或子目录中");
					return;
				}
				const srcDir = parentDir(src);
				const sameDir = srcDir !== null && (state.platform === "win32" ? srcDir.toLowerCase() === dirPath.toLowerCase() : srcDir === dirPath);
				if (sameDir) return; // already there
			...
			function bindTreeDropTargets() {
				...
				treeEl.addEventListener("dragover", (ev) => {
					if (dragSrcPath === null || ev.target !== treeEl) return;
					ev.preventDefault();
					try { ev.dataTransfer.dropEffect = "move"; } catch (err) {}
				});
				treeEl.addEventListener("dragenter", (ev) => {
					if (dragSrcPath === null || ev.target !== treeEl) return;
					ev.preventDefault();
					clearDropTarget();
					dropTargetEl = treeEl;
				...
				treeEl.addEventListener("dragleave", (ev) => {
					if (dropTargetEl === treeEl) clearDropTarget();
				});
