...
`;
		...
		async function apply(ctx) {
			...
			function ensureDock() {
				...
				/* v1.10.1: provider 卸载/重载（HMR、插件重装）感知——ui-beautify
				   的纤维重建会提供【新】的 dock 服务对象，旧 dockApiRef 指向已
				   失效闭包（订阅不到新 engine notify、注册面板也随新 host 丢失）。
				   发现服务身份变化即放弃旧绑定并重新注册，否则入口按钮永远隐藏、
				   面板永不登入插件面板（症状：ui-beautify 重载后「文件浏览器」
				   入口消失，直到本插件自身重载）。 */
				if (dockApiRef !== undefined && d !== undefined && d !== dockApiRef) {
