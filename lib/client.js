...
.fexp-panel:not([data-fexp-theme="dark"]) .hljs { color: #24292f; background: transparent; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-keyword, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-selector-tag, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-literal, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-meta .hljs-keyword, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-doctag { color: #cf222e; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-string, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-regexp, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-addition, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-attribute { color: #0a3069; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-number, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-symbol { color: #0550ae; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-comment, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-quote { color: #6e7781; font-style: italic; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-title, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-title.function_, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-section { color: #8250df; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-title.class_, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-type { color: #953800; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-built_in, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-params { color: #953800; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-variable, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-template-variable { color: #24292f; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-attr, .fexp-panel:not([data-fexp-theme="dark"]) .hljs-attribute { color: #0550ae; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-deletion { color: #82071e; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-meta { color: #0550ae; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-emphasis { font-style: italic; }
.fexp-panel:not([data-fexp-theme="dark"]) .hljs-strong { font-weight: 600; }
/* --- hljs 配色：暗色（github-dark 风格） --- */
.fexp-panel[data-fexp-theme="dark"] .hljs { color: #c9d1d9; background: transparent; }
.fexp-panel[data-fexp-theme="dark"] .hljs-keyword, .fexp-panel[data-fexp-theme="dark"] .hljs-selector-tag, .fexp-panel[data-fexp-theme="dark"] .hljs-literal, .fexp-panel[data-fexp-theme="dark"] .hljs-doctag { color: #ff7b72; }
.fexp-panel[data-fexp-theme="dark"] .hljs-string, .fexp-panel[data-fexp-theme="dark"] .hljs-regexp, .fexp-panel[data-fexp-theme="dark"] .hljs-addition { color: #a5d6ff; }
.fexp-panel[data-fexp-theme="dark"] .hljs-number, .fexp-panel[data-fexp-theme="dark"] .hljs-symbol { color: #79c0ff; }
.fexp-panel[data-fexp-theme="dark"] .hljs-comment, .fexp-panel[data-fexp-theme="dark"] .hljs-quote { color: #8b949e; font-style: italic; }
.fexp-panel[data-fexp-theme="dark"] .hljs-title, .fexp-panel[data-fexp-theme="dark"] .hljs-section { color: #d2a8ff; }
.fexp-panel[data-fexp-theme="dark"] .hljs-title.class_, .fexp-panel[data-fexp-theme="dark"] .hljs-type { color: #ffa657; }
.fexp-panel[data-fexp-theme="dark"] .hljs-built_in, .fexp-panel[data-fexp-theme="dark"] .hljs-params { color: #ffa657; }
.fexp-panel[data-fexp-theme="dark"] .hljs-attr { color: #79c0ff; }
.fexp-panel[data-fexp-theme="dark"] .hljs-attribute { color: #ffa657; }
.fexp-panel[data-fexp-theme="dark"] .hljs-deletion { color: #ffa198; }
.fexp-panel[data-fexp-theme="dark"] .hljs-meta { color: #79c0ff; }
.fexp-panel[data-fexp-theme="dark"] .hljs-emphasis { font-style: italic; }
.fexp-panel[data-fexp-theme="dark"] .hljs-strong { font-weight: 600; }
`;
		...
		const CONTRIBUTION = {
			...
			descriptors: [
				...
				{
					id: "dsh-file-explorer#fileExplorer/fsReveal",
					service: "fileExplorer",
					namespace: "fileExplorer",
					method: "fsReveal",
					invocation: { kind: "direct" },
					parameters: [
						{ name: "path", wire: "path", source: "json", codec: strictCodec("dsh-file-explorer#fileExplorer/fsReveal:path") }
					],
					result: strictCodec("dsh-file-explorer#fileExplorer/fsReveal:result"),
					sourceLocation: { "file": "dsh-file-explorer/lib/client.js", "line": 1, "column": 1 }
				},
				{
		...
		async function apply(ctx) {
			...
			function remote() {
				return {
					...
					fsReveal: (path) => call("fsReveal", path),
			...
			function buildSearchBar() {
				...
				input.addEventListener("keydown", (ev) => {
					...
					else if (ev.key === "ArrowRight") { ev.preventDefault(); toggleDir(state.root); }
				});
			...
			function buildLineView(p, htmlLines) {
				...
				numCol.style.cssText = "position:sticky;left:0;z-index:1;background:var(--dsw-alias-bg-layer-2);";
				const lineCol = document.createElement("div");
				lineCol.className = "fexp-codelines fexp-code-font";
				scroll.appendChild(numCol);
				scroll.appendChild(lineCol);
				...
				const renderRange = (first, count) => {
					...
					for (let i = 0; i < count; i += 1) {
						...
						nd.textContent = String(n + 1);
						fragN.appendChild(nd);
						const ld = document.createElement("div");
						ld.className = "fexp-cline";
						ld.style.minHeight = LINE_H + "px";
						if (htmlLines) ld.innerHTML = htmlLines[n] !== undefined ? htmlLines[n] : "";
						else ld.textContent = lines[n] !== undefined ? lines[n] : "";
						fragL.appendChild(ld);
					}
					numCol.appendChild(fragN);
					lineCol.appendChild(fragL);
				};
				if (lines.length <= VIRTUALIZE_THRESHOLD) {
					renderRange(0, lines.length);
				} else {
					// 虚拟化：只渲染可视窗口 ± 缓冲行；行高固定保证滚动位置精确
					const onScroll = () => {
						const first = Math.max(0, Math.floor(scroll.scrollTop / LINE_H) - 25);
						const vis = Math.ceil(scroll.clientHeight / LINE_H) + 50;
						renderRange(first, Math.min(vis, lines.length - first));
					};
				...
				numCol.style.minWidth = (16 + digits * 8) + "px";
				// 截断续读：底部「继续加载」
			...
			function renderPreview() {
				...
				previewEl.appendChild(buildPreviewHead(p));
			...
			function buildEditor() {
				...
				const numCol = document.createElement("div");
				numCol.className = "fexp-linenums fexp-code-font";
				numCol.style.cssText = "flex:none;overflow:hidden;height:100%;box-sizing:border-box;";
				const frag = document.createDocumentFragment();
			...
			/** v1.9.0 在系统文件管理器中显示目标（平台分发在服务端）。 */
			function revealInFileManager(path) {
				remote().fsReveal(path).then(() => {
					showToast("已在系统文件管理器中显示");
				}).catch((err) => {
					showToast("打开失败：" + ((err && err.message) || err));
				});
			}
			function onContextMenu(ev, path, type, name) {
				...
				if (type === "dir") {
					...
					items.push(menuSep());
					items.push(menuItem("在资源管理器中显示", "external", () => revealInFileManager(path)));
			...
			function toggleHelp() {
				...
				const x = rect ? rect.left + rect.width / 2 - 180 : window.innerWidth / 2 - 180;
				const y = rect ? Math.max(8, rect.top + 8) : Math.max(8, window.innerHeight / 2 - 160);
			...
			function themeNow() {
				...
				for (const el of document.querySelectorAll(".fexp-panel")) {
			...
			ctx.effect(function globalOverlayHandlers() {
				...
				const onDocKey = (ev) => {
					...
					if (ev.key === "Escape") {
						...
						if (state.editing) { closeEditor(); renderPreview(); ev.preventDefault(); }
						return;
					}
