...
.fexp-menu-item.fexp-danger { color: #e5484d; }
.fexp-menu-item.fexp-danger:hover { background: rgba(229,72,77,.16); }
...
.fexp-row.fexp-drop-target, .fexp-tree.fexp-drop-target { outline: 2px solid rgba(90,140,255,.7); outline-offset: -1px; background: rgba(90,140,255,.12); }
...
`;
		...
		async function apply(ctx) {
			...
			let hostMode = false;         // dock host (card mode) owns the panel surface
			...
			function applyFloat() {
				if (floatEl) floatEl.style.display = (hostMode || !state.showFloat) ? "none" : "inline-flex";
			...
			const DOCK_TOP_INSET = 76; // header: 12px pad-top + 32px titleRow + 4px margin + 27px tabs — pinned to 0.1.0-rc.6 bundles
			 ...
			 * buttons / grips / float-ball toggle); `host=false` is the classic
			...
			function restoreStandaloneRefs() {
				...
				floatBtn = standaloneRefs.floatBtn;
			}
			...
			/** Hide/show the classic-panel surface and the floating ball for the
			 * current mode. Called on every dock-state transition. */
			...
			/* --- v1.6: dock host integration (ui-beautify card mode) --- */
			/** True while ui-beautify's card engine actually owns the frame
			 * (`data-vsc-layout` stamped). Guards against the engine being loaded
			 * but unable to find its frame, in which case the dock host renders
			 * nothing and the classic panel must stay in charge. */
			function frameHasVscLayout() {
				try {
					const root = document.querySelector('[data-slot="root"]');
					return root !== null && root.firstElementChild !== null && root.firstElementChild.hasAttribute("data-vsc-layout");
			...
			function onDockChange() {
				...
				const card = (typeof d.mode === "function" ? d.mode() : "card") === "card";
				const nextHost = card && frameHasVscLayout();
				if (nextHost === hostMode) {
					/* same mode: only sync the button highlight when the HOST panel
					   ...
					   In classic mode the host state is latent — it must not
					   re-open the classic panel behind the user's back. */
				...
				hostMode = nextHost;
				if (hostMode) {
					/* entering card mode: carry the classic open state into the host
					   panel (open if it was open, close if it was closed). */
					...
					/* leaving card mode: host state carries over to the classic panel;
					   re-point refs in case they live in a now-unmounted host tree. */
