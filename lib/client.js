...
`;
		...
		async function apply(ctx) {
			 ...
			 */
			function canDropInto(dirPath) {
				if (dragSrcPath === null || !dirPath) return false;
				if (isSelfOrDescendant(dragSrcPath, dirPath)) return false;
				const srcDir = parentDir(dragSrcPath);
				if (srcDir !== null) {
			...
			function onDrop(ev, dirPath) {
				...
				clearDropTarget();
				dragSrcPath = null;
				if (!canDropInto(dirPath)) return; // native-cursor path already prevented, but guard anyway
