...
/** v1.10.1: 按字符边界安全截断，避免切断 UTF-16 代理对（emoji/部分 CJK 扩展区）。
 *  窗口首尾出现孤立代理字符会渲染成 �。切断处落在代理对中间时回退 1 字符。 */
...
function makeMarkdownRenderer(toc, seen) {
  return {
    ...
    link(token) {
      ...
      // href 经白名单校验后仍需转义：markdown 链接文本可携带 `"`，不转义会
      // 突破属性边界注入事件处理器（如 [x](https://a/"onmouseover="alert(1))）。
      return `<a href="${escapeHtml(safe)}"${external ? ' target="_blank" rel="noopener"' : ''}>${inner}</a>`
    },
    ...
    // text 来自内联 token，可能含 raw HTML（如 `# <img src=x onerror=…>`），
    // 必须转义——标题不是 Markdown 输出管线，不能依赖 html() 的兜底转义。
    heading(token) {
      ...
      return `<h${token.depth} id="${id}">${escapeHtml(text)}</h${token.depth}>`
    },
    // 代码块：按语言高亮（未知语言自动检测）
    code(token) {
  ...
  for (const chunk of chunks) {
    if (skipped < offset) {
      ...
      if (rest !== '') {
        ...
        text += rest.length > need ? safeSlice(rest, need) : rest
