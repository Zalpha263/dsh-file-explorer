...
class FileExplorerService extends TypertRemoteService {
  ...
  async fsReveal(path) {
    ...
    if (process.platform === 'win32') {
      ...
      await runShell('explorer', ['/select,' + display], 'explorer failed')
    } else if (process.platform === 'darwin') {
      await runShell('open', ['-R', display], 'open -R failed')
    } else {
      await runShell('xdg-open', [dirname(display)], 'xdg-open failed')
    }
    return { ok: true }
  }
