...
/* v1.11.1：渲染管线（marked + 3.3MB 的 highlight.js）改为按需加载。
   DSH 启动时会 import 本插件的 Host 半区，静态 import './render.js' 会让每次
   启动都白付一次解析成本（实测约 +200ms）；预览/读取是首次真正需要时才用到
   的功能，所以在第一次使用时动态 import 并缓存同一个 promise。 */
let renderModulePromise = null
function renderLib() {
  if (renderModulePromise === null) renderModulePromise = import('./render.js')
  return renderModulePromise
}

/* v1.11.1：让宿主 fs 写入与【会话策略】一致，而不是落回部署默认策略。
   本插件的边界检查按会话 cwd 判定，宿主沙箱也应按同一会话的 mode/root 裁决；
   否则会话设为 read-only（或 cwd 与部署 fallback root 不同）时，写入会被错误
   地放行 / 拒绝。找不到任何会话时退回无参 resolve()（部署默认）。 */
...
class FileExplorerService extends TypertRemoteService {
    ...
    try {
      ...
      if (typeof fs.streamText === 'function') {
        ...
        for await (const chunk of stream) {
          ...
          text += chunk.length > need ? R.safeSlice(chunk, need) : chunk
        }
    ...
    try {
      ...
      if (stream !== null) {
        ...
        for await (const chunk of stream) {
          if (acc >= cap) break
          const need = cap - acc
          chunks.push(chunk.length > need ? R.safeSlice(chunk, need) : chunk)
          acc += chunk.length > need ? need : chunk.length
        }
      } else {
        chunks.push(await fs.readText(target))
      }
      const windowed = R.windowText(chunks, offset, limit)
      ...
      const kindInfo = offset > 0
        ...
        : R.detectKind(basename(fs.processPath(target)), text)
      ...
      if (kindInfo.kind === 'markdown') {
        const rendered = R.renderMarkdown(text)
        // text 一并返回：客户端「源码」视图与编辑模式需要原始文本
        return Object.assign(base, { html: rendered.html, toc: rendered.toc, text })
      }
      if (kindInfo.kind === 'code' && lineCount <= R.MAX_RENDER_LINES) {
        return Object.assign(base, { lines: R.highlightLines(text, kindInfo.language), text })
      }
  ...
  async fsWrite(path, content, expectedVersion) {
    ...
    if (typeof expectedVersion === 'string' && expectedVersion !== '') {
      ...
      try {
        const outcome = await fs.writeText(target, content, { kind: 'replaceIfVersion', version: expectedVersion }, undefined, sandboxPolicyFor(this.ctx))
    ...
    try {
      const outcome = await fs.writeText(target, content, undefined, undefined, sandboxPolicyFor(this.ctx))
      const after = await fs.stat(target)
      return { path: fs.processPath(target), version: outcome.version, size: after ? after.size : null }
    } catch (err) {
      if (err && err.code === 'FS_SANDBOX_DENIED') {
  ...
  async fsCreate(path, type, content) {
    ...
    if (fs !== undefined) {
      ...
      await ensureParentDir(filePath)
      try {
        await fs.writeText(target, typeof content === 'string' ? content : '', { kind: 'createIfAbsent' }, undefined, sandboxPolicyFor(this.ctx))
        const info = await fs.stat(target)
        return { path: fs.processPath(target), type: 'file', size: info ? info.size : null }
      } catch (err) {
        if (err && err.code === 'FS_SANDBOX_DENIED') {
          throw new Error('当前会话的沙箱策略不允许创建该文件：' + filePath)
        }
