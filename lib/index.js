...
import { detectKind, renderMarkdown, highlightLines, windowText, MAX_RENDER_LINES } from './render.js'
...
/**
 * 启动系统 GUI 程序（explorer / open / xdg-open）且不等待退出。
 * Windows 的 explorer 常驻时新实例会立即退出并返回退出码 1（把请求委托
 * 给已有实例），因此这类调用只能以「进程创建成功」为准，不能校验退出码。
 */
...
class FileExplorerService extends TypertRemoteService {
  ...
  /**
   * v1.9.0 渲染预览：按文件类型返回 Markdown 富文本 / 逐行高亮代码 / 纯文本。
   * 支持 offset 续读（「继续加载」）：offsetChars 为字符偏移（续读时从断点
   * 继续流式读取），offsetLines 为已读行数（客户端行号累计）。
    ...
    try {
      ...
      if (stream !== null) {
        ...
        for await (const chunk of stream) {
          if (acc >= cap) break
          const need = cap - acc
          chunks.push(chunk.length > need ? chunk.slice(0, need) : chunk)
      ...
      if (truncated) {
        ...
        try {
          if (stream !== null) {
            const full = typeof fs.streamText === 'function' ? await fs.streamText(target) : null
            if (full !== null) {
              for await (const chunk of full) {
                for (let i = 0; i < chunk.length; i += 1) {
                  if (chunk.charCodeAt(i) === 10) total += 1
                }
              }
            }
          }
        } catch (err) { /* 统计失败则缺省 */ }
        return Object.assign(base, { totalLines: total + (total > 0 ? 1 : 0), text })
      }
  ...
  /**
   * v1.9.0 在系统文件管理器中显示目标（Windows 选中 / macOS Finder 显示 /
   * Linux 打开所在目录）。失败抛出可读错误，由客户端 toast 展示。
   */
  async fsReveal(path) {
    if (typeof path !== 'string' || path.trim() === '') throw new Error('path is required')
    const target = await this.ctx.get('fs').resolve(path)
    const info = await this.ctx.get('fs').stat(target)
    if (info === undefined) throw new Error('not found: ' + path)
    // 用 processPath 输出（与 fsList/fsRead 一致的用户路径），而非 displayPath
    const display = this.ctx.get('fs').processPath(target)
    if (process.platform === 'win32') {
      // explorer 需要反斜杠路径；dsh 的 processPath 可能输出正斜杠
      const winPath = display.replace(/\//g, '\\')
      // explorer 官方语法 `explorer /select,<path>`（逗号紧跟路径，合并单参数）；
      // 以进程创建成功为准（常驻 explorer 的新实例退出码为 1，属正常）
      await launchDetached('explorer', ['/select,' + winPath])
    } else if (process.platform === 'darwin') {
      await launchDetached('open', ['-R', display])
    } else {
      await launchDetached('xdg-open', [dirname(display)])
    }
    return { ok: true }
  }
